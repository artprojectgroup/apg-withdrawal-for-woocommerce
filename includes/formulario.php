<?php
/**
 * Página de ajustes del plugin WC - APG Withdrawal.
 *
...
<div class="wrap woocommerce">
	<h2><?php esc_html_e( 'APG Withdrawal Options.', 'wc-apg-withdrawal' ); ?></h2>
	...
	<p><?php esc_html_e( 'Add to WooCommerce an online withdrawal workflow with customer form, My Account integration and admin request log.', 'wc-apg-withdrawal' ); ?></p>
	...
	<form id="formulario" method="post" action="options.php">
			...
			<tr valign="top">
				<th scope="row" class="titledesc">
					<label for="apg_withdrawal_settings[notification_email]">
						<?php esc_html_e( 'Notification email', 'wc-apg-withdrawal' ); ?>
					</label>
				</th>
			...
			<tr valign="top">
				<th scope="row" class="titledesc">
					<label for="apg_withdrawal_settings[page_id]">
						<?php esc_html_e( 'Withdrawal page', 'wc-apg-withdrawal' ); ?>
					</label>
				</th>
				<td class="forminp">
					<?php
					echo wp_kses(
						wp_dropdown_pages(
							array(
								...
								'echo'              => 0,
								'show_option_none'  => __( 'Select a page', 'wc-apg-withdrawal' ),
								'option_none_value' => '0',
							)
						),
						array(
					...
					?>
					<p class="description"><?php printf( wp_kses( /* translators: %s: shortcode tag */ __( 'Page where the shortcode %s is published.', 'wc-apg-withdrawal' ), array( 'code' => array() ) ), '<code>[apg_withdrawal_form]</code>' ); ?></p>
				</td>
			</tr>
			<tr valign="top">
				<th scope="row" class="titledesc">
					<label for="apg_withdrawal_settings[button_text]">
						<?php esc_html_e( 'Confirmation button text', 'wc-apg-withdrawal' ); ?>
					</label>
				</th>
				...
				<tr valign="top">
					<th scope="row" class="titledesc">
						<label for="apg_withdrawal_settings[withdrawal_days]">
							<?php esc_html_e( 'Withdrawal window (days)', 'wc-apg-withdrawal' ); ?>
						</label>
					</th>
					<td class="forminp">
						...
						<p class="description"><?php esc_html_e( 'Orders older than this number of days will not show the withdrawal button. The form still accepts late requests with a legal warning.', 'wc-apg-withdrawal' ); ?></p>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row" class="titledesc">
						<label for="apg_withdrawal_settings[grace_days]">
							<?php esc_html_e( 'Extra grace days', 'wc-apg-withdrawal' ); ?>
						</label>
					</th>
				...
				<tr valign="top">
					<th scope="row" class="titledesc">
						<label for="apg_withdrawal_settings[deadline_source]">
							<?php esc_html_e( 'Deadline source', 'wc-apg-withdrawal' ); ?>
						</label>
					</th>
					<td class="forminp">
						<select id="apg_withdrawal_settings[deadline_source]" name="apg_withdrawal_settings[deadline_source]" tabindex="<?php echo esc_attr( $tab ); $tab++; ?>">
							<option value="completed" <?php selected( $apg_withdrawal_settings['deadline_source'], 'completed' ); ?>><?php esc_html_e( 'Completed date', 'wc-apg-withdrawal' ); ?></option>
							<option value="created" <?php selected( $apg_withdrawal_settings['deadline_source'], 'created' ); ?>><?php esc_html_e( 'Order created date', 'wc-apg-withdrawal' ); ?></option>
						</select>
						<p class="description"><?php esc_html_e( 'Use completed date as the default legal approximation. If the order is not completed yet, the plugin will fall back automatically.', 'wc-apg-withdrawal' ); ?></p>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row" class="titledesc">
						<label for="apg_withdrawal_settings[store_ip]">
							<?php esc_html_e( 'Store IP address?', 'wc-apg-withdrawal' ); ?>
						</label>
				</th>
			...
			<tr valign="top">
				<th scope="row" class="titledesc">
					<label for="apg_withdrawal_settings[store_user_agent]">
						<?php esc_html_e( 'Store browser identifier?', 'wc-apg-withdrawal' ); ?>
					</label>
				</th>
			...
			<tr valign="top">
				<th scope="row" class="titledesc">
					<label for="apg_withdrawal_settings[delete_data_on_uninstall]">
						<?php esc_html_e( 'Delete all data on uninstall?', 'wc-apg-withdrawal' ); ?>
					</label>
				</th>
				<td class="forminp">
					...
					<p class="description"><?php esc_html_e( 'If checked, all withdrawal requests, their metadata and the plugin settings will be permanently deleted when the plugin is uninstalled. Leave unchecked to keep your data.', 'wc-apg-withdrawal' ); ?></p>
				</td>
			</tr>
		</table>
		...
		<h3><?php esc_html_e( 'Automation', 'wc-apg-withdrawal' ); ?></h3>
		<p><?php esc_html_e( 'Automatically update withdrawal request status when the linked WooCommerce order changes status.', 'wc-apg-withdrawal' ); ?></p>
		<table class="form-table apg-table">
			...
			<tr valign="top">
				<th scope="row" class="titledesc">
					...
					printf(
						...
						esc_html__( 'Mark as %s when order becomes', 'wc-apg-withdrawal' ),
				...
				<td class="forminp">
					...
					<select
						...
						style="width:350px"
						data-placeholder="<?php esc_attr_e( 'Select order statuses&hellip;', 'wc-apg-withdrawal' ); ?>"
						aria-label="<?php esc_attr_e( 'Order status', 'wc-apg-withdrawal' ); ?>"
					...
					<?php else : ?>
					<em><?php esc_html_e( 'WooCommerce order statuses not available.', 'wc-apg-withdrawal' ); ?></em>
					<?php endif; ?>
				</td>
			</tr>
			<?php endforeach; ?>
		...
		<h3><?php esc_html_e( 'Customer email notifications', 'wc-apg-withdrawal' ); ?></h3>
		<p>
			<?php esc_html_e( 'Send an email to the customer when the withdrawal request status changes. Configure content in', 'wc-apg-withdrawal' ); ?>
			<a href="<?php echo esc_url( admin_url( 'admin.php?page=wc-settings&tab=email' ) ); ?>"><?php esc_html_e( 'WooCommerce &rsaquo; Settings &rsaquo; Emails', 'wc-apg-withdrawal' ); ?></a>.
		</p>
		<table class="form-table apg-table">
			<?php foreach ( $withdrawal_statuses as $w_status => $w_label ) : // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedVariableFound -- Local template variables in included file ?>
			<tr valign="top">
				<th scope="row" class="titledesc">
					<label for="apg_withdrawal_settings_email_<?php echo esc_attr( $w_status ); ?>">
						...
						printf(
							/* translators: %s withdrawal status label. */
							esc_html__( 'Email when changed to %s', 'wc-apg-withdrawal' ),
							'<strong>' . esc_html( $w_label ) . '</strong>'
						);
						?>
					</label>
